...
def __get(key, default=None):
    ...
    if setting:
        return __get_setting_value(setting)
    else:
        return default


def __contains_id(setting_id):
...
def get_v2_core_xray():
    return __get("v2_core_xray", False)
...
def init_db(update=False):
    ...
    add_if_not_exist(
        Setting("is_traffic_reset", "is_traffic_reset", "False", "bool", "", True), update
    )
    add_if_not_exist(
